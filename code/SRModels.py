...
class Model():
    ...
    def __init__(self,projectdir='/home/ti61/f_mc1689_1/SRActFlow/',ruletype='12',n_hiddenregions=10,randomize=False,scratchfcdir=None):
        ...
        motor_ind = np.asarray(motor_ind)
    ...
    def loadRealMotorResponseActivations(self):
    ...
    def simulateGroupActFlow(self,thresh=0,nproc='max'):
        """
    ...
    def actflowDecoding(self,trainset,testset,outputfile,
                        nbootstraps=1000,nproc='max',null=False,verbose=True):
        if nproc=='max':
            nproc=mp.cpu_count()
        ...
        for i in range(nbootstraps):
            ...
            distances_baseline[0,:],rmatch,rmismatch, confusion_mats = tools.actflowDecodings(testset,trainset,
                                                                                              effects=True, featsel=False,confusion=True,permutation=null,
                                                                                              ncvs=1, nproc=nproc)
