...
def maxT(diff_arr, nullmean=0, alpha=.05, tail=1, permutations=1000, nproc=1, pvals=False):
    ...
    if tail == 1:
        ...
        # for two-sided tests, the returned dist is 2d
        maxT_dist_sorted = np.sort(maxT_dist[:,0]) #max/upperhalf
        minT_dist_sorted = np.sort(maxT_dist[:,1]) #min/bottomhalf
        topPercVal_maxT_inx = int(len(maxT_dist_sorted)*(1.0-alpha/2.0)) 
        botPercVal_maxT_inx = int(len(minT_dist_sorted)*(alpha/2.0)) 
        # Provide two r thresholds for two-tailed test 
        topT_thresh = maxT_dist_sorted[topPercVal_maxT_inx]
        botT_thresh = minT_dist_sorted[botPercVal_maxT_inx]
