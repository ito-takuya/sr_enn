function output = loadStimFiles_neutralStimuliByTrial(subj, gsr)
% Taku Ito
% 01/13/2017
%
...
%   gsr - 1 if you would like to include a GSR regressor in the matrix, 0 if not   
    ...
    numTaskStims = 4; % 4 stimuli (neutral); localizers
    ...
    for stim=1:numTaskStims
        stimdir = '/projects3/StroopActFlow/data/stimfiles/NeutralStimuli_Localizers/';
    ...
    stimMat2 = zeros(totalTRs,numTaskStims*nTrialsPerStim);
    trialCount = 1;
    for stim=1:numTaskStims
        tr = 1;
        for trial=1:nTrialsPerStim
            % once 2 TRs are taken up by a column, go to next trial
            while sum(stimMat2(:,trialCount))<2 
                % is this TR a stim?
