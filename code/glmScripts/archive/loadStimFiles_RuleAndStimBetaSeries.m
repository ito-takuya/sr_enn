function output = loadStimFiles_RuleAndStimBetaSeries(subj, gsr)
% Taku Ito
% 02/21/2017
%
...
%   gsr - 1 if you would like to include a GSR regressor in the matrix, 0 if not   
    ...
    numEVs = 600; % 60 task rule encodings; 540 stimulus presentations/trials
    numMotionParams = 12; % HCP Pipe outputs 12
    trLength = .785; 
    %nBlocksPerRule = 30; % 30 blocks of each rule type (word v. color)
    ...
    stimMat = zeros(totalTRs,numEVs);
    for stim=1:numEVs
        stimdir = '/projects3/StroopActFlow/data/stimfiles/RuleStimByTrial/';
...
%    % Separate out stims per trial
%    % first 30 are stim 1; 2nd 30 are stim2
%    stimMat2 = zeros(totalTRs,numEVs*nBlocksPerRule);
%    trialCount = 1;
%    for stim=1:numEVs
%        tr = 1;
%        for trial=1:nBlocksPerRule
%            % once 2 TRs are taken up by a column, go to next trial
%            while sum(stimMat2(:,trialCount))<7 % 7 TRs including instruction + delay
%                % is this TR a stim?
%                if stimMat(tr,stim)==1
%                    stimMat2(tr,trialCount) = 1;
%                end
%                tr = tr + 1;    
%            end
...
%    end
%
%    % Reset stim variables to be stims by trial
%    stimMat = stimMat2;
%    taskStims = stimMat;
