...
%   gsr - 1 if you would like to include a GSR regressor in the matrix, 0 if not   
    ...
    trLength = .785; 
    numTasks = 8;
    stimdir = ['/projects2/ModalityControl2/data/stimfiles_miniblock_encodingVprobe_v3/'];
...
%    eval(['!1d_tool.py -overwrite -infile ' analysisdir '/' subj '_wholebrainsignal_timeseries_rest.1D -derivative -write ' analysisdir '/' subj '_wholebrainsignal_timeseries_deriv_rest.1D'])
    ...
    % First 128 stims will be encoding blocks
    num_encode_stims = 128;
    stimMat = [];
    for stim=1:num_encode_stims
        %disp(['Loading all mini blocks for encoding'])
        stimfilename = dir([stimdir subj '_stimfile_MiniblockBetaSeriesEncodingVProbe_ModControl_EV*_MiniblockEncoding' num2str(stim) '.1D']);
%         stimfilename = dir([stimdir subj '_stimfile_Unique64TaskSet_IndivRITL_EV' num2str(stim) '_*']);
    ...
    % Second 128 stims will be probe blocks
    num_probe_stims = 128;
    stimMat = [];
    for stim=1:num_probe_stims
        %disp(['Loading all mini blocks for probes'])
        stimfilename = dir([stimdir subj '_stimfile_MiniblockBetaSeriesEncodingVProbe_ModControl_EV*_MiniblockProbe' num2str(stim) '.1D']);
%         stimfilename = dir([stimdir subj '_stimfile_Unique64TaskSet_IndivRITL_EV' num2str(stim) '_*']);
        tmp = importdata([stimdir stimfilename.name]);
        stimMat = [stimMat tmp];
    end
