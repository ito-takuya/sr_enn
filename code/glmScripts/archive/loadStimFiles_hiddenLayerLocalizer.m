function output = loadStimFiles_hiddenLayerLocalizer(subj, gsr)
% Taku Ito
% 02/26/2017
%
...
% Regressing out WM, Ventricles (and corresponding derivatives), motion parameters, linear trend, and 4 S-R associations for each rule context (hidden layer localizers)
% 
% Parameters: 
...
%   gsr - 1 if you would like to include a GSR regressor in the matrix, 0 if not   
    ...
    numTaskStims = 4; % 4 hidden layer units (using neutral trials only to localize)
    ...
    for stim=1:numTaskStims
        stimdir = '/projects3/StroopActFlow/data/stimfiles/HiddenLayer_Localizers/';
