...
# Originally taken and adapted from IndivRITL script
# RUBIC protocol -- Stroop ActFlow preprocessing script using HCP Pipelines 
...
Usage: hcp_preprocessing_func.sh [options]
    ...
    --orthanc=<"true">          (optional) Input "true", if data was downloaded from orthanc server and needs to be renamed and reorganized
...
if [ "$server" == "nm3" ]; then
    basedir="/scratch/ti61/projects/StroopActFlow"
else
...
######################### Added Taku 11/17/15
# Rename and organize subject directories from RUBIC's annoying constant organization changes...
if [ -z "$orthanc" ]; then
    echo "Skipping orthanc set up"
elif [ "$orthanc" == "true" ]; then
    echo "Renaming files due to orthanc"
    pushd ${rawdatadir} 
    # First remove the first three characters of each sequence directory
    for file in MR*; do mv "$file" "${file:3}"; done

    # Now add a -1 to the first run of the task sequences
    mv cmrr_mbep2d_bold_2mm_MB8_Task_8x cmrr_mbep2d_bold_2mm_MB8_Task_8x-1
    mv cmrr_mbep2d_bold_2mm_MB8_Task_8x_SBRef cmrr_mbep2d_bold_2mm_MB8_Task_8x_SBRef-1
...
### SPECIFY SCAN LISTS ###
# Anatomicals
T1w=(`ls ${rawdatadir}/ | grep t1_mpr_ns_sag_p2_iso_32Channel || true;` )
T2w=(`ls ${rawdatadir}/ | grep t2_spc_sag_p2_iso || true;` )
# Spin Echo Maps/Gradient Field Maps
SE_AP=(`ls ${rawdatadir}/ | grep cmrr_mbep2d_se_AP || true;` ) # AP is the NEGATIVE PHASE encoding (think a 
SE_PA=(`ls ${rawdatadir}/ | grep cmrr_mbep2d_se_PA || true;` ) # PA is the POSITIVE PHASE encoding
GRE_FIELDMAP=(`ls ${rawdatadir}/ | grep gre_field_mapping || true;` )
# Multiband EPI sequences 
# Rest scans
RestEPIs=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_Rest || true;` ) # This returns an array of the scans delimited by spaces
# SBRefs for Rest scans
RestEPIs_SBRef=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_Rest_SBRef || true;` )
# Task scans
TaskEPIs=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_Task_8x-[0-9] || true;` )
# SBRefs for task scans
TaskEPIs_SBRef=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_Task_8x_SBRef-[0-9] || true;` )
# Task localizer
#TaskLoc=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_TaskLoc || true;` )
# SBRef for task localizer
#TaskLoc_SBRef=(`ls ${rawdatadir}/ | grep mbep2d_bold_2mm_MB8_TaskLoc_SBRef || true;` )
...
unwarpdir="-y" # unwarp direction for A >> P phase encoded data is -y (think about how anterior to posterior coordinate direction is -y). It follows that unwarpdir for P >> A collected data would be "y", but we have not collected this data for the StroopActFlow study.
...
if [ -z "$anatomicalRecon" ]; then
    ...
    mri_convert ${rawdatadir}/${T1w}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/T1w/T1w.nii.gz
    echo "Reconstructing T2 image"
    mri_convert ${rawdatadir}/${T2w}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/T2w/T2w.nii.gz
    echo "Reconstructing Spin Echo Field Maps"
    mri_convert ${rawdatadir}/${SE_AP}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/SE_Maps/SE_AP.nii.gz
    mri_convert ${rawdatadir}/${SE_PA}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/SE_Maps/SE_PA.nii.gz
    echo "Reconstructing Gradient Field Map"
    mri_convert ${rawdatadir}/${GRE_FIELDMAP}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/GRE_FieldMap/GRE_FieldMap.nii.gz
...
if [ -z "$epiRecon" ]; then
    ...
    for ((i=0;i<${#RestEPIs[@]};++i)); do
        ...
        mri_convert ${rawdatadir}/${RestEPIs[i]}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/Rest${count}/Rest${count}.nii.gz
        mri_convert ${rawdatadir}/${RestEPIs_SBRef[i]}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/Rest${count}/Rest${count}_SBRef.nii.gz
    ...
    for ((i=0;i<${#TaskEPIs[@]};++i)); do
        ...
        # reconstruct using mri_convert (freesurfer) for both the epi scan and the sbref
        mri_convert ${rawdatadir}/${TaskEPIs[i]}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/Task${count}/Task${count}.nii.gz
        mri_convert ${rawdatadir}/${TaskEPIs_SBRef[i]}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/Task${count}/Task${count}_SBRef.nii.gz
...
#    mri_convert ${rawdatadir}/${TaskLoc}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/TaskLoc/TaskLoc.nii.gz
#    mri_convert ${rawdatadir}/${TaskLoc_SBRef}/*0000.dcm --in_type siemens --out_type nii ${unprocesseddir}/TaskLoc/TaskLoc_SBRef.nii.gz
...
if [ -z "$concatenateRuns" ]; then
    ...
    echo "Concatenating the 6 task runs for subject ${subj}..."
    runs="1 2 3 4 5 6" 
    runList=""
    for run in $runs
    do
